...
sudo apt-get install -y software-properties-common curl git npm
...
# Установка nginx-proxy и настройка на проброс 80 (HTTP) и 443 (HTTPS) портов
...
sudo docker run -d --name nginx-letsencrypt \
 --restart=always \
 --volumes-from nginx-proxy \
 --network reverse-proxy \
 -v $HOME/certs:/etc/nginx/certs:rw \
 -v /var/run/docker.sock:/var/run/docker.sock:ro jrcs/letsencrypt-nginx-proxy-companion
